...
static uint8_t otp_ss_key[ID2_OTP_SS_KEY_LEN] = {0};

#if ((CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_AES) || defined(CONFIG_ID2_OTP_X_MODE))
...
static int _id2_sm4_ecb(const uint8_t* key, uint32_t key_len,
                const uint8_t* in, uint32_t in_len,
                uint8_t* out, uint32_t* out_len, bool is_enc)
{
    int ret = 0;
                              ...
                              uint8_t* rept_data, uint32_t* rept_len)
{
    ...
    id2_plat_get_random(otp_ss_key, ID2_OTP_SS_KEY_LEN);
    ...
    offset += 1;
    memcpy(rept_data + offset, otp_ss_key, ID2_OTP_SS_KEY_LEN);
...
static int _otp_set_otp_data(uint8_t* otp_data, uint32_t len)
{
    ...
    kcv_data_len = ID2_ROUNDUP(ID2_ID_MIN_LEN + 1, AES_BLOCK_SIZE);
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_3DES)
    ...
    kcv_data_len = ID2_ROUNDUP(ID2_ID_MIN_LEN + 1, DES_BLOCK_SIZE);
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM4)
    if (key_type == KEY_TYPE_SM4_128) {
        key_size = 16;
    }
    ...
    kcv_data_len = ID2_ROUNDUP(ID2_ID_MIN_LEN + 1, AES_BLOCK_SIZE);
#else
    #error("OTP not supported for this key type");
#endif

    otp_data_len = 1 + ID2_ID_MIN_LEN + key_data_len + kcv_data_len;
    if (len != otp_data_len) {
        id2_log_error("otp len is not match: %d %d\n", len, otp_data_len);
        return IROT_ERROR_GENERIC;
    }

    key_data_off = 1 + ID2_ID_MIN_LEN;
    kcv_data_off = 1 + ID2_ID_MIN_LEN + key_data_len;
...
#if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_AES) || defined(CONFIG_ID2_OTP_X_MODE)
    ret = _id2_aes_ecb(otp_ss_key, ID2_OTP_SS_KEY_LEN,
...
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_3DES)
    ret = _id2_des3_ecb(otp_ss_key, ID2_OTP_SS_KEY_LEN,
                   otp_data + key_data_off, key_data_len,
...
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM4)
    ret = _id2_sm4_ecb(id2_key, key_size,
                       otp_data + kcv_data_off, kcv_data_len,
                       id2_id, &kcv_data_len, false);
    if (ret < 0) {
        id2_log_error("id2 id decrypt fail\n");
    ...
    id2_log_hex_dump("id2_id", id2_id, ID2_ID_MIN_LEN);

    if (!memcmp(id2_id, (uint8_t*)otp_data + 1, ID2_ID_MIN_LEN)) {
        ...
        } else if (key_type == KEY_TYPE_SM4_128) {
        ...
        km_ret = km_set_id2(id2_id, ID2_ID_MIN_LEN);
...
{
    uint8_t id2[ID2_ID_MAX_LEN];
    uint32_t len = ID2_ID_MAX_LEN;
    uint32_t km_ret;
        ...
        uint8_t* auth_code, uint32_t* len)
{
    ...
    /* magic */
    if (token[0] != 'i' && token[0] != 'x') {
        id2_log_error("invalid token magic: %c\n", token[0]);
        ret = IROT_ERROR_GENERIC;
        goto _out;
    }
...
#if !defined(CONFIG_ID2_OTP_X_MODE)
...
#else /* !CONFIG_ID2_OTP_X_MODE */

#if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM4)
    key_type = KEY_TYPE_SM4_128;
    key_size = 16;
    block_size = SM4_BLOCK_SIZE;
#else
    #error("not supported for this key type");
#endif
...
irot_result_t id2_client_load_otp_data(const uint8_t* otp_data, uint32_t otp_data_len)
{
    ...
    ret = _otp_set_otp_data((uint8_t*)otp_data, otp_data_len);
    if (ret != IROT_SUCCESS) {
