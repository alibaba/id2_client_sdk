...
#define ID2_LOCAL_DEVICE_RANDOM_LEN         16
...
static void _dump_id2_conf_info(void)
{
    ...
    if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_3DES) {
        ...
        ls_osa_print("CONFIG_ID2_KEY_TYPE: %s\n", "ID2_KEY_TYPE_RSA");
    } else if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_ECC) {
        ls_osa_print("CONFIG_ID2_KEY_TYPE: %s\n", "ID2_KEY_TYPE_ECC");
    } else if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM1) {
        ls_osa_print("CONFIG_ID2_KEY_TYPE: %s\n", "ID2_KEY_TYPE_SM1");
    } else if (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM2) {
        ls_osa_print("CONFIG_ID2_KEY_TYPE: %s\n", "ID2_KEY_TYPE_SM2");
...
static void _id2_inverse(uint8_t* id2, uint32_t id_len)
{
    uint8_t i = 0;
    uint8_t hex = 0;
...
static irot_result_t _id2_verify(uint8_t* sign_in, uint32_t sign_in_len,
                                 uint8_t* sign_out, uint32_t sign_out_len)
{
    irot_result_t ret = IROT_SUCCESS;
    uint8_t out_data[MAX_SIGN_LEN];
    uint32_t out_len = MAX_SIGN_LEN;

    ret = _id2_sign(sign_in, sign_in_len, out_data, &out_len);
    if (ret != IROT_SUCCESS) {
        id2_log_error("id2 sign error.\n");
        return ret;
    }

    if (out_len != sign_out_len) {
        id2_log_error("invalid sign length, %d %d.\n", out_len, sign_out_len);
        return IROT_ERROR_GENERIC;
    }

    if (memcmp(out_data, sign_out, sign_out_len) != 0) {
...
{
    irot_result_t ret = IROT_SUCCESS;
    uint8_t hash_buf[MAX_HASH_LEN];
    uint32_t hash_len = MAX_HASH_LEN;
    ...
    km_ret = km_sign(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                     &sign_params, hash_buf, hash_len, sign_out, sign_out_len);
    if (km_ret != KM_SUCCESS) {
                                 ...
                                 uint8_t* sign_out, uint32_t sign_out_len)
{
    ...
    km_ret = km_verify(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                       &sign_params, hash_buf, hash_len, sign_out, sign_out_len);
    if (km_ret != KM_SUCCESS) {
        id2_log_error("km verify fail, 0x%08x\n", km_ret);
        return IROT_ERROR_GENERIC;
    }
...
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_ECC)
                                ...
                                uint8_t* out, uint32_t* out_len)
{
    ...
    enc_params.padding_type = KM_NO_PADDING;

    km_ret = km_asym_decrypt(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                             &enc_params, in, in_len, out, out_len);
    if (km_ret != KM_SUCCESS) {
        id2_log_error("km_asym_decrypt fail, 0x%x\n", km_ret);
        return IROT_ERROR_GENERIC;
    }
                               ...
                               uint8_t* sign_out, uint32_t* sign_out_len)
{
    ...
    sign_params.padding_type = KM_NO_PADDING;
    sign_params.digest_type = KM_SHA256;

    id2_log_hex_dump("id2 sign input:", sign_in, sign_in_len);

    km_ret = km_msg_sign(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                &sign_params, NULL, 0, sign_in, sign_in_len, sign_out, sign_out_len);
    if (km_ret != KM_SUCCESS) {
        id2_log_error("km msg sign fail, 0x%x\n", km_ret);
        return IROT_ERROR_GENERIC;
    }
                                 ...
                                 uint8_t* sign_out, uint32_t sign_out_len)
{
    ...
    km_ret = km_msg_verify(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                &sign_params, NULL, 0, sign_in, sign_in_len, sign_out, sign_out_len);
    if (km_ret != KM_SUCCESS) {
        id2_log_error("km msg verify fail, 0x%x\n", km_ret);
        return IROT_ERROR_GENERIC;
    }
...
#elif (CONFIG_ID2_KEY_TYPE == ID2_KEY_TYPE_SM2)
                               ...
                               uint8_t* sign_out, uint32_t* sign_out_len)
{
    ...
    sign_params.digest_type = KM_SM3;
    sign_params.padding_type = KM_NO_PADDING;

    id2_log_hex_dump("id2 sign input:", sign_in, sign_in_len);

    km_ret = km_msg_sign(ID2_KEY_NAME, ID2_KEY_NAME_LEN,
                    &sign_params, (uint8_t *)id, strlen(id),
                    sign_in, sign_in_len, sign_out, sign_out_len);
    if (km_ret != KM_SUCCESS) {
        id2_log_error("km msg sign fail, 0x%08x\n", km_ret);
        return IROT_ERROR_GENERIC;
    }
                                        ...
                                        uint8_t* auth_code, uint32_t* auth_code_len)
{
    ...
    uint8_t id2_str[ID2_ID_MAX_LEN + 1] = {0};
    char device_random_str[ID2_LOCAL_DEVICE_RANDOM_LEN + 1] = {0};
    ...
    sign_in_len = id2_len + strlen(device_random_str) + strlen(server_random) + extra_len;
...
irot_result_t id2_client_get_id(uint8_t* id, uint32_t* len)
{
    ...
    if (s_id2_id_len == 0) {
        ...
        if (*len > ID2_ID_MAX_LEN) {
            id2_log_error("id2 excess limit, %d %d\n", *len, ID2_ID_MAX_LEN);
            return IROT_ERROR_GENERIC;
        }

        /* For special legacy case (not suggest),
        ...
        memcpy(id, s_id2_id, s_id2_id_len);
        *len = s_id2_id_len;
    }
                         ...
                         const uint8_t* server_extra, uint32_t server_extra_len)
{
    ...
    if (ret != IROT_SUCCESS) {
        ...
        /* inverse id2 */
        _id2_inverse(id2_str, id2_len);
    }

    id2_log_debug("inversed id2: %s\n", id2_str);

    /* id2 signature */
    head = tail + 1;
    sign_in_len = id2_len + device_random_len + server_extra_len;
    sign_in = (uint8_t*)ls_osa_malloc(sign_in_len + 1);
    if (sign_in == NULL) {
        id2_log_error("out of mem, %d.\n", sign_in_len + 1);
        return IROT_ERROR_OUT_OF_MEMORY;
    } else {
        memcpy(sign_in, id2_str, id2_len);
        memcpy(sign_in + id2_len, device_random, device_random_len);
    }
    ...
    if (auth_code_type == TYPE_SERVER_CHALLENGE_WITH_EXTRA) {
        ...
        memcpy(sign_in + id2_len + device_random_len, server_extra, server_extra_len);
    }

    sign_out_len = MAX_SIGN_LEN;
    sign_out = ls_osa_malloc(sign_out_len);
    if (sign_out == NULL) {
        id2_log_error("out of mem, %d\n", sign_out_len);
        ret = IROT_ERROR_OUT_OF_MEMORY;
        goto _out;
    ...
    ret = id2_plat_base64_decode(head, length, sign_out, &sign_out_len);
    if (ret < 0) {
    ...
    ret = _id2_verify(sign_in, sign_in_len, sign_out, sign_out_len);
    if (ret != IROT_SUCCESS) {
        id2_log_error("id2 verify signature fail, %d\n", ret);
        goto _out;
    }
...
irot_result_t id2_client_set_id2_and_key(const char* id2, int key_type, const char* key_value)
{
    ...
    km_ret = km_set_id2((uint8_t*)id2, id2_len);
    if (km_ret != KM_SUCCESS) {
