...
#if defined(MBEDTLS_SSL_PROTO_ITLS_TEST)
    if (ssl->conf->type == ITLS_TEST_FLAGS_CLI_HELLO) {
        if (ssl_tst_count++ == 1 && ssl->out_msg[0] == MBEDTLS_SSL_HS_CLIENT_HELLO) {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "TLS TEST - Discard One Client Hello!!!\n\n" ) );
            return( 0 );
        }
    } else if (ssl->conf->type == ITLS_TEST_FLAGS_CLI_FINISHED) {
        if (ssl_tst_count == 0 && ssl->out_msg[0] == MBEDTLS_SSL_HS_FINISHED) {
            ssl_tst_count++;
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "TLS TEST - Discard One Client Finished!!!\n\n" ) );
            return( 0 );
        }
    }
#endif
...
#if defined(MBEDTLS_SSL_PROTO_ITLS_TEST)
                if (ssl->conf->type == ITLS_TEST_FLAGS_CLI_HELLO ||
                    ssl->conf->type == ITLS_TEST_FLAGS_CLI_FINISHED) {
                    MBEDTLS_SSL_DEBUG_MSG( 1, ( "TLS TEST - Resend flight one more!!!\n\n" ) );
                    if( ( ret = mbedtls_ssl_resend( ssl ) ) != 0 )
                    {
                        MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_resend", ret );
                        return( ret );
...
#if defined(MBEDTLS_SSL_PROTO_ITLS)
static int ssl_change_cipher_spec( mbedtls_ssl_context *ssl )
{
    /*
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length, %d\n", ssl->in_msglen ) );
        return( MBEDTLS_ERR_SSL_INVALID_RECORD );
    }
    ...
    if( ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE )
    {
        if( ssl->state == MBEDTLS_SSL_SERVER_FINISHED )
        {
            ...
            {
                if( ssl_change_cipher_spec( ssl ) != 0 )
                {
                    MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad change cipher spec" ) );
...
#if defined(MBEDTLS_SSL_PROTO_ITLS)
        /* Drop unexpected HandShake messages */
        if( ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
            ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER &&
            ssl->handshake == NULL )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "dropping unexpected HandShake" ) );
            return( MBEDTLS_ERR_SSL_UNEXPECTED_RECORD );
        }
#endif
...
int mbedtls_ssl_handle_message_type( mbedtls_ssl_context *ssl )
{
    ...
    {
#if defined(MBEDTLS_SSL_PROTO_ITLS)
        message_alert_type = ssl->in_msg[1];
...
#if defined(MBEDTLS_SSL_PROTO_ITLS_TEST)
    if (ssl->conf->type == ITLS_TEST_DATA_VERIFY_DATA)
    {
         MBEDTLS_SSL_DEBUG_MSG( 0, ( "TLS TEST - Invalid Client Finished Verify Data!!!\n\n" ) );

         memcpy(ssl->out_msg + 4, ssl->conf->verify_data, ssl->conf->verify_data_len);
    }
#endif
...
#if defined(MBEDTLS_SSL_PROTO_ITLS_TEST)
    if (ssl->conf->type == ITLS_TEST_FLAGS_SRV_FINISHED) {
        MBEDTLS_SSL_DEBUG_MSG( 1, ("TLS TEST - Parse Server Finished Fail!!!\n\n" ) );
        mbedtls_ssl_send_alert_message( ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                        MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR );
        return( MBEDTLS_ERR_SSL_INTERNAL_ERROR );
    }
...
#if defined(MBEDTLS_SSL_PROTO_ITLS)
    if( ( ret = id2_client_init() ) != IROT_SUCCESS )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "id2_client_init failed, %d", ret ) );
        return ret;
    }

    message_alert_type = 0;
#endif
...
{
    if( auth_extra == NULL || auth_extra_len == 0 )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    if( auth_extra_len > MBEDTLS_AUTH_EXTRA_MAX_LEN )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    conf->auth_extra = (unsigned char *)auth_extra;
    conf->auth_extra_len = auth_extra_len;
...
{
    if( auth_token == NULL || auth_token_len == 0 )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    if( auth_token_len > MBEDTLS_AUTH_TOKEN_MAX_LEN )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    conf->auth_token = (unsigned char *)auth_token;
    conf->auth_token_len = auth_token_len;
...
#if defined(MBEDTLS_SSL_PROTO_ITLS)
    mbedtls_free( handshake->challenge );
    id2_client_cleanup();
#endif
...
static int ssl_preset_suiteb_ciphersuites[] = {
    MBEDTLS_TLS_ID2_WITH_AES_256_CBC_SHA256,
    MBEDTLS_TLS_ID2_WITH_AES_256_CBC_SHA,
    MBEDTLS_TLS_ID2_WITH_AES_128_CBC_SHA256,
    MBEDTLS_TLS_ID2_WITH_AES_128_CBC_SHA,
    0
};

static int ssl_preset_default_ciphersuites[] = {
    MBEDTLS_TLS_ID2_WITH_AES_256_CBC_SHA256,
    MBEDTLS_TLS_ID2_WITH_AES_256_CBC_SHA,
    MBEDTLS_TLS_ID2_WITH_AES_128_CBC_SHA256,
    MBEDTLS_TLS_ID2_WITH_AES_128_CBC_SHA,
                                 ...
                                 int endpoint, int transport, int preset )
{
    ...
    switch( preset )
    {
        ...
        default:
            ...
            conf->ciphersuite_list[MBEDTLS_SSL_MINOR_VERSION_3] =
#if defined(MBEDTLS_SSL_PROTO_ITLS)
                                   ssl_preset_default_ciphersuites;
#else
