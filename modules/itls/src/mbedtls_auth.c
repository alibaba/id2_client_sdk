        ...
        mbedtls_ssl_context *ssl, unsigned char *buf, size_t *olen)
{
    ...
    *p++ = (unsigned char)((MBEDTLS_TLS_EXT_KEY_ID) & 0xFF);
    ...
    {
        SSL_DBG_LOG("TLS TEST - Invalid CLient Hello Key Group(0x%x)!!!\n\n", ssl->conf->key_group);
    ...
    {
         SSL_DBG_LOG("TLS TEST - Invalid Client Hello Key ID!!!\n\n");
    ...
    *p++ = (unsigned char)((MBEDTLS_TLS_EXT_AUTH_CODE) & 0xFF);
...
#endif /* CONFIG_KEY_OTP_ENABLED */
    {
        if (ssl->handshake->challenge != NULL &&
            ...
            {
                SSL_DBG_LOG("TLS TEST - Invalid Client Hello Auth Code!!!\n\n");
        ...
        mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    if (ssl->conf->type == ITLS_TEST_FLAGS_SRV_VERIFY) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("TLS TEST - Parse Hello Verify Fail!!!\n\n"));
        return -1;
    }
#endif

    if (len < 4) {
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "no extension data for hello verify " ) );
        return 0;
    }
        ...
        mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    ssl->handshake->auth_code_verified = 1;
    ret = 0;
